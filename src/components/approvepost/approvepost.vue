...
<script>
import { getAllProduct,UpdateProduct } from "../../api";
...
export default {
  ...
  methods: {
    async getAllUser(page) {
      const result = await getAllProduct({ page, rows: 8, state: 1 }, "get");
    ...
    async approve(item) {
      await UpdateProduct({id:item.id,state:0},'put')
       this.getAllUser(this.page); //点击第几页
    },

     async deny(item) {
       await UpdateProduct({id:item.id,state:2},'put')
       this.getAllUser(this.page); //点击第几页
    },
